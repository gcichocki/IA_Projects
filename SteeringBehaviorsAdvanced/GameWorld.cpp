...
//------------------------------- Nos différents setup possibles pour lancer l'appli ---------------------------------
void LeaderPoursuiveurs(GameWorld * world, int* cx, int* cy, CellSpacePartition<Vehicle*> * m_pCellSpace, std::vector<Vehicle*>* m_Vehicles) {
	/////////////////////	Déclaration de nos agents ///////////////////////////

	//Declaration de l'agent leader
	AgentLeader *pVehicle = new AgentLeader(world,
		Vector2D(500, 500),                 //initial position
		...
		FALSE);					//AI 
	pVehicle->SetScale(Vector2D(10, 10));
	pVehicle->SetMaxSpeed(70);
	m_Vehicles->push_back(pVehicle);
	//add it to the cell subdivision
	m_pCellSpace->AddEntity(pVehicle);
	gdi->SetPenColor(2); //vert

						 //Declaration du deuxieme agent leader
	AgentLeader *pVehicle2 = new AgentLeader(world,
		Vector2D(400, 500),                 //initial position
		RandFloat()*TwoPi,        //start rotation
		Vector2D(0, 0),            //velocity
		Prm.VehicleMass,          //mass
		Prm.MaxSteeringForce,     //max force
		...
		Prm.VehicleScale,			//scale
		TRUE);					//AI  
	pVehicle2->SetScale(Vector2D(10, 10));
	pVehicle2->SetMaxSpeed(70);
	m_Vehicles->push_back(pVehicle2);
	//add it to the cell subdivision
	m_pCellSpace->AddEntity(pVehicle2);

	//Declaration des agents poursuiveurs
	for (int a = 1; a<20; ++a)
	{
		...
		AgentPoursuiveur *pVehicle = new AgentPoursuiveur(world,
...
void SneakyHide(GameWorld * world, int* cx, int* cy, CellSpacePartition<Vehicle*> * m_pCellSpace, std::vector<Vehicle*>* m_Vehicles) {
	/////////////////////	Déclaration de nos agents ///////////////////////////

	//determine a random starting position
	Vector2D SpawnPos = Vector2D(*cx / 2.0 + RandomClamped()*(*cx) / 2.0,
		*cy / 2.0 + RandomClamped()*(*cy) / 2.0);
	Vehicle* pHunter = new Vehicle(world,
		SpawnPos,                 //initial position
		RandFloat()*TwoPi,        //start rotation
		Vector2D(0, 0),            //velocity
		Prm.VehicleMass,          //mass
		...
		Prm.MaxTurnRatePerSecond, //max turn rate
		Prm.VehicleScale);        //scale
	pHunter->SetScale(Vector2D(10, 10));
	pHunter->SetMaxSpeed(70);
	pHunter->Steering()->WanderOn();
	pHunter->Steering()->ObstacleAvoidanceOn();
	m_Vehicles->push_back(pHunter);
	...
	Vehicle* pVehicle = new Vehicle(world,
		SpawnPos,                 //initial position
		RandFloat()*TwoPi,        //start rotation
		Vector2D(0, 0),            //velocity
		Prm.VehicleMass,          //mass
	...
	pVehicle->SetScale(Vector2D(7,7));
	pVehicle->Steering()->SneakOn(pHunter);
...
void FlockingV(GameWorld * world, int* cx, int* cy, CellSpacePartition<Vehicle*> * m_pCellSpace, std::vector<Vehicle*>* m_Vehicles) {
	/////////////////////	Déclaration de nos agents ///////////////////////////
...
void AppliBase(GameWorld * world, int* cx, int* cy, CellSpacePartition<Vehicle*> * m_pCellSpace, std::vector<Vehicle*>* m_Vehicles) {
	/////////////////////	Déclaration de nos agents ///////////////////////////
            ...
            m_bShowCellSpaceInfo(false)
{
  ...
  //DECOMMENTER LA LIGNE CI-DESSOUS POUR LE SCENARIO DE BASE
  //AppliBase(this, &cx, &cy, m_pCellSpace, &m_Vehicles);

  //DECOMMENTER LA LIGNE CI-DESSOUS POUR LE PREMIER SCENARIO LEADER POURSUIVEURS
  //LeaderPoursuiveurs(this, &cx, &cy, m_pCellSpace, &m_Vehicles);
  
  //DECOMMENTER LA LIGNE CI-DESSOUS POUR LE SCENARIO DE FLOCKINGV
  //FlockingV(this, &cx, &cy, m_pCellSpace, &m_Vehicles);
  
  //DECCOMENTER LESS DEUX LIGNES CI-DESSOUS POUR LE SCENARIO DE SNEAKY HIDE
    SneakyHide(this, &cx, &cy, m_pCellSpace, &m_Vehicles);
    CreateObstacles();
