...
{       
  ...
  if (On(sneak))
  {
	  ...
	  force = Sneak(m_pTargetAgent1, m_pVehicle->World()->Obstacles()) * m_dWeightHide;
...
{        
  ...
  if (On(sneak))
  {
	  ...
	  m_vSteeringForce += Sneak(m_pTargetAgent1, m_pVehicle->World()->Obstacles()) * m_dWeightHide;
  }
...
//
//------------------------------------------------------------------------
Vector2D SteeringBehavior::Sneak(const Vehicle* hunter,	const vector<BaseGameEntity*>& obstacles)
{
	...
	while (curOb != obstacles.end())
	{
		...
		//spot to the agent
		double dist = Vec2DDistanceSq(HidingSpot, m_pVehicle->Pos()) + Vec2DDistanceSq(HidingSpot, hunter->Pos());
...
//  be determined by the user inputs
//------------------------------------------------------------------------
Vector2D SteeringBehavior::MoveVehicle()
{
	Vector2D DesiredVelocity = Vector2D(0, 0);
	if (KEYDOWN('Z')) 
		DesiredVelocity += Vector2D(0, -1);
	if (KEYDOWN('S'))
		DesiredVelocity += Vector2D(0, 1);
	if (KEYDOWN('Q'))
		DesiredVelocity += Vector2D(-1, 0);
	if (KEYDOWN('D'))
		DesiredVelocity += Vector2D(1, 0);
	
	DesiredVelocity = Vec2DNormalize(DesiredVelocity)
		* m_pVehicle->MaxSpeed();
...
{ 
  ...
  Vector2D target = m_vWanderTarget +Vector2D(m_dWanderDistance*1, 0);
...
Vector2D SteeringBehavior::FlockingV(const vector<Vehicle*> &neighbors)
{
	...
	// Seuil de distance maximale pour déterminer si un agent est trop loin de ses voisins
	double seuilDistanceMax = 50;
	// Seuil distance minimale pour déterminer si un agent est trop proche de son voisin le plus proche
	double seuilDistanceMin = 30;
	// Distance pour laquelle on considère que la vision d'un agent est obstruée
	double seuilDistanceVision = 100;
	// Vitesse minimale des agents lorsqu'ils doivent ralentir pour s'éloigner de leur voisin le plus proche
	double minSpeed = 8;
	// Variables utiles à la détermination du voisin le plus proche du sujet
	double ClosestNeighborDistance = 0;
	int IndClosestNeighborDistance = 0;


	// On introduit un agent "référence" qui sert de boussole au groupe d'agent : il décide de la direction à prendre et
	// possède une vitesse de croisière
	if (neighbors[0] == m_pVehicle) {
		m_pVehicle->SetMaxSpeed(20);
		return Wander();
	}
	// Pour les autres agents, on initialise la valeur de voisin le plus proche
	// en prenant soin qu'ils ne se considèrent pas eux-mêmes
	// Le voisin 1 prend donc le voisin 2 pour initialisation
	else if (neighbors[1] == m_pVehicle)
	{
		ClosestNeighborDistance = Vec2DLength(m_pVehicle->Pos() - neighbors[2]->Pos());
		IndClosestNeighborDistance = 2;
	}
	// Les autres voisins prennent le voisin 1 pour initialisation
	else
	{
		ClosestNeighborDistance = Vec2DLength(m_pVehicle->Pos() - neighbors[1]->Pos());
		IndClosestNeighborDistance = 1;
	}

	// On recherche ensuite la distance au voisin le plus proche
	for (unsigned int a = 0; a<neighbors.size(); ++a)
	{
		if ((neighbors[a] != m_pVehicle))
		{
			double dist = Vec2DLength(m_pVehicle->Pos() - neighbors[a]->Pos());
			if (dist < ClosestNeighborDistance) {
				ClosestNeighborDistance = dist;
				IndClosestNeighborDistance = a;
			}
		}
	}

	// On vérifie si les 3 règles sont respectées pour le sujet
	// Vérification Règle 1
	if (ClosestNeighborDistance > seuilDistanceMax)
	{
		rule1 = false;
	}

	// Vérification Règle 3
	if ((ClosestNeighborDistance < seuilDistanceMin) && (m_pVehicle->Heading().Dot(neighbors[IndClosestNeighborDistance]->Pos() - m_pVehicle->Pos())>0))
	{
		rule3 = false;
	}

	// Vérification Règle 2
	// On regarde si sa vue est obstruée et on retient l'indice du voisin gêneur
	boolean SomeoneInTheWay = false;
	int IndNeighborInTheWay = 0;
	for (int a = 0; a < neighbors.size(); ++a)
	{
		// On utilise un produit scalaire entre le heading du sujet et le vecteur le séparant du voisin analysé couramment
		// et on vérifie que l'éventuel gêneur est bien dans le champs de vision du sujet
		if ((Vec2DNormalize(m_pVehicle->Heading()).Dot(Vec2DNormalize(neighbors[a]->Pos() - m_pVehicle->Pos())) > 0.80)
			&& ((Vec2DDistance(neighbors[a]->Pos(), m_pVehicle->Pos())) < seuilDistanceVision)) {
			SomeoneInTheWay = true;
			IndNeighborInTheWay = a;
			rule2 = false;
		}
	}

	// Si les 3 règles sont respectées, le sujet adopte sa vitesse et direction de croisière
	// en observant le voisin 0, la référence
	if (rule1 && rule2 && rule3)
	{
		m_pVehicle->SetHeading(neighbors[0]->Heading());
		m_pVehicle->SetVelocity(neighbors[0]->Velocity());
	}



	// Regle 1 : si un agent est trop loin des autres agents, il accélère pour se rapprocher du plus proche
	if (!rule1) {
		{
			m_pVehicle->SetVelocity((neighbors[IndClosestNeighborDistance]->Pos() - m_pVehicle->Pos()) * 10);
		}

	}

	// Regle 3 : si un agent est trop proche d'un autre, il ralentit
	if (!rule3) {
		// Si l'agent est derrière son voisin trop proche, il ralentit
		if ((m_pVehicle->Heading().Dot(neighbors[IndClosestNeighborDistance]->Pos() - m_pVehicle->Pos())>0)
			&& m_pVehicle->Velocity().Length() > minSpeed) {
			m_pVehicle->SetVelocity(m_pVehicle->Velocity()*0.99);
		}

		return SteeringForce;
	}

	// Regle 2 : si un agent est suffisamment près d’un autre, il va venir sur l’un de ses côtés pour que sa vue ne soit pas obstruée;
	if (!rule2) {
		Vector2D Direction = Vec2DNormalize(neighbors[IndNeighborInTheWay]->Side());
		SteeringForce = Seek(neighbors[IndNeighborInTheWay]->Pos() + 60 * Direction);
		return SteeringForce;
	}
