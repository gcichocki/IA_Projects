...
private:
  ...
  // The agent will try to follow the hunter by choosing obstacle close to the hunter in its back
  Vector2D Sneak(const Vehicle* hunter, const std::vector<BaseGameEntity*>& obstacles);

  //You can move the agent with you keyboard zqsd
  Vector2D MoveVehicle();
...
public:
  ...
  void SneakOn(Vehicle* v) { m_iFlags |= sneak; m_pTargetAgent1 = v; }
  void MoveAgentOn() { m_iFlags |= moveagent; }
  ...
  // Code rajoute pour le TP : permet de mettre en place un Flocking en V
  void FlockingVOn() { m_iFlags |= flockingV; }

  void SneakOff() { if (On(sneak)) m_iFlags ^= sneak; }
  void MoveAgentOff() { if (On(moveagent))   m_iFlags ^= moveagent; }
  ...
  // Code rajoute pour le TP : permet de mettre en place un Flocking en V
  void FlockingVOff() { if(On(flockingV)) m_iFlags ^= flockingV; }


  bool isSneakOn() { return On(sneak); }
  bool isMoveAgentOn() { return On(moveagent); }
  ...
  bool isFlockingVOn() { return On(flockingV); }
